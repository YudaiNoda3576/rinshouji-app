...
// 一覧のページサイズ（サーバーサイドページネーション）。
const PAGE_SIZE = 20;

// 一覧クエリと同じ FROM/WHERE で総件数のみを取得するフォールバック。
// page が総ページ数を超えると OFFSET により rows が空になり COUNT(*) OVER() が
// 1 行も返らないため、その場合にこちらで正しい total を求める。
// WHERE が hm/d のエイリアスを参照するため LEFT JOIN は 3 つとも維持する。
async function countHouseholds(conditions: string[], params: unknown[]): Promise<number> {
  ...
  return toNum(rows[0]?.total_count) ?? 0;
}
...
householdsRoute.get("/", async (c) => {
  ...
  if (pageRaw !== undefined) {
    const parsedPage = parseId(pageRaw);
    if (parsedPage === null) {
      return c.json({ error: "page が不正です" }, 400);
    }
    page = parsedPage;
  }
  ...
  const { rows } = await pool.query(sql, limitParams);

  // rows が空でも page > 1 の場合は「範囲外ページ」の可能性があるため、
  // フォールバックで総件数を取得する（page === 1 で空なら本当に 0 件）。
  const total =
    rows.length > 0
      ? (toNum(rows[0].total_count) ?? 0)
      : page > 1
        ? await countHouseholds(conditions, params)
        : 0;
