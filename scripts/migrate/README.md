...
- `npm run migrate:transform` … フェーズ②のみ(import_records → 本番テーブル)
- 接続先の変更: `DATABASE_URL=postgres://user:pass@host:5432/dbname npm run migrate -- ...`

## 冪等性

再実行のたびに import_records(①)と本番 6 テーブル(②)を TRUNCATE してから
入れ直す洗い替え方式。何度実行しても結果は同一になる。
...
src/transform-phase.ts   フェーズ②: オーケストレーション(洗い替え)
